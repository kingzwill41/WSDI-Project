<?php
   ...
   if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST["email"])) {
      ...
      if(!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $_SESSION["emailErr"] = "The email you entered is incorrect";
      }else{
    ...
    if (empty($_POST["password"])) {
     ...
     {
         $_SESSION["passwordErr"] = null;
         $_SESSION["password"] = $password; 
     }

     if(!isset($_POST['loginType'])) { 
         $_SESSION["loginTypeErr"] = "Please select a user type"; 
     } else {
         $loginType = test_input($_POST["loginType"]);
         $_SESSION["loginTypeErr"] = null;
   ...
   $query = "SELECT * FROM staff WHERE Email='$username' && Password='$password' && Type='$loginType' ";
   ...
   {     
        ...
        if (strcmp($_SESSION["loginType"], "admin") ){
            $_SESSION["password"]=null;
            $_SESSION["loginType"]=null;
            $_SESSION["email"]=null;
            header("location: view/admin/dashboard.php");
        }else if (strcmp($_SESSION["loginType"], "doctor") ){
            $_SESSION["password"]=null;
            $_SESSION["loginType"]=null;
            $_SESSION["email"]=null;
            header("location: view/doctor/dashboard.php");
        }else if (strcmp($_SESSION["loginType"], "nurse") ){
            $_SESSION["password"]=null;
            $_SESSION["loginType"]=null;
            $_SESSION["email"]=null;
            header("location: view/nurse/dashboard.php");
        }

        if( isset($_SESSION["emailErr"]) || isset($_SESSION["loginTypeErr"]) || isset($_SESSION["passwordErr"])){
   ...
   else
   {
    ...
    if(isset($_SESSION['email']) && isset($_SESSION['loginType']) && isset($_SESSION['password'])){
        $_SESSION["error"] = "";
    }else{
        session_destroy();
    }
